...
import Database.MySQL.Base (ConnectInfo, connectPort, connectUser, connectPath,
                            defaultConnectInfo, connectDatabase)
import           Fmt ((+|), (|+))
...
toConnectInfo db = defaultConnectInfo {
  connectPort = port db,
  connectUser = user db,
  connectPath = socket db,
  connectDatabase = ""
  }
